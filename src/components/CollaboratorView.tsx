...
import { getBrazilCurrentMinuteOfDay, getLocalTodayString, getDuration, matchesAppointmentSearch } from './appointmentUtils';
import { SCHEDULE_CONFIG } from '../config/scheduleConfig';
import { SupervisorClientesTab } from './SupervisorClientesTab';
...
export function CollaboratorView({
  ...
  const [myScheduleDays, setMyScheduleDays] = useState<string[]>((currentUser.diasTrabalho || '1,2,3,4,5,6').split(','));
  const WEEK_DAYS = [
    { value: '0', label: 'DOM' },
    { value: '1', label: 'SEG' },
    { value: '2', label: 'TER' },
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={`bg-surface-container-lowest p-6 lg:p-8 rounded-3xl border border-outline-variant/10 shadow-sm overflow-hidden flex flex-col ${activeTab === 'agenda' ? '' : 'hidden'}`}>
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-6 gap-4 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
            ...
            <button onClick={() => openBlockModal('', currentUser.id)} className="bg-error text-onError px-3 py-2 rounded-xl text-[11px] sm:text-xs font-bold uppercase tracking-widest transition-all shadow-sm hover:bg-error/90 text-white flex items-center gap-1 whitespace-nowrap">
            ...
            <button onClick={() => setShowMyScheduleModal(true)} className="bg-surface-container-low text-on-surface px-3 py-2 rounded-xl text-[11px] sm:text-xs font-bold uppercase tracking-widest transition-all shadow-sm hover:bg-surface-container-high border border-outline-variant/20 flex items-center gap-1 whitespace-nowrap">
              <span className="material-symbols-outlined text-[18px]">schedule</span> Minha Escala
            </button>
        ...
        <div className="flex-1 bg-surface-container-lowest border border-outline-variant/20 rounded-2xl overflow-hidden flex flex-col shadow-sm h-[70vh]">
          ...
          <div className="flex flex-1 overflow-y-auto relative custom-scrollbar">
            {/* Coluna da Timeline */}
            <div className="w-14 shrink-0 bg-surface-container-lowest/95 backdrop-blur border-r border-outline-variant/20 sticky left-0 z-10 flex flex-col">
            ...
            <div className="relative flex-1 z-10" onClick={(e) => handleGridClick(e, currentUser.id)} style={{ height: (END_HOUR - START_HOUR) * HOUR_HEIGHT }}>
              ...
              {myBlockers.map(bloq => {
                ...
                return (
                  <div key={bloq.id} className="absolute left-2 right-2 bg-surface-container-high/60 backdrop-blur-sm rounded-lg border border-outline-variant/30 flex items-center justify-center group overflow-hidden" style={{ top, height }}>
                    ...
                    {onDeleteBloqueio && (
                      <button onClick={() => onDeleteBloqueio(bloq.id)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 p-1 text-on-surface hover:text-error bg-surface shadow-sm rounded-full z-20 transition-all"><span className="material-symbols-outlined text-[14px]">delete</span></button>
                    )}
                  </div>
                );
              })}
              ...
              {filteredAppointmentsForSelectedDate.map(app => {
                ...
                return (
                  <div key={app.id} onClick={(e) => { e.stopPropagation(); handleEditAppointmentClick(app); }} className={`absolute left-2 right-2 rounded-xl border ${cardStatusClass} flex flex-col group overflow-hidden transition-all hover:z-20 cursor-pointer`}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-surface-container-low p-6 rounded-3xl border border-outline-variant/10 shadow-2xl w-full max-w-lg" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="space-y-4 max-h-[72vh] overflow-y-auto pr-1 custom-scrollbar">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {WEEK_DAYS.map((day) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setMyScheduleDays(prev => prev.includes(day.value) ? prev.filter(item => item !== day.value) : [...prev, day.value].sort());
                      }}
                      className={`rounded-xl border px-2 py-2 text-[10px] font-bold uppercase tracking-widest transition-all ${myScheduleDays.includes(day.value) ? 'bg-primary text-on-primary border-primary' : 'bg-surface-container-lowest text-on-surface-variant border-outline-variant/20'}`}
                    >
                      {day.label}
                    </button>
                  ))}
              ...
              <button 
                type="button" 
                onClick={async () => {
                  // Para colaborador, apenas atualizar seus próprios diasTrabalho
                  if (onEditAppointment) {
                    // Se houver forma de atualizar perfil, usar aqui
                    // Por enquanto, apenas fecha o modal
                    setToastMessage('Escala atualizada!');
                    setShowMyScheduleModal(false);
                    setTimeout(() => setToastMessage(null), 3000);
                  }
                }}
                className="w-full bg-primary text-on-primary py-3 rounded-xl font-bold text-xs uppercase tracking-[0.2em] shadow-sm hover:bg-primary-dim transition-colors"
              >
                Salvar Minha Escala
              </button>
              <button type="button" onClick={() => setShowMyScheduleModal(false)} className="w-full py-2 rounded-xl text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:bg-surface-container">
                Cancelar
              </button>
            </div>
          </motion.div>
