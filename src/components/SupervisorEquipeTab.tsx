...
export function SupervisorEquipeTab({ employees, onAddEmployee, onDeleteEmployee, onEditEmployee, setToastMessage, setErrorMessage }: SupervisorEquipeTabProps) {
    ...
    try {
      if (editingEmployeeId) {
        success = await onEditEmployee(editingEmployeeId, {
          name: newEmpName, email: newEmpEmail, specialty: newEmpSpecialty, role: newEmpRole, diasTrabalho: newEmpDiasTrabalho.join(',')
        });
        ...
        success = await onAddEmployee({
          name: newEmpName, email: generatedEmail, role: newEmpRole, specialty: newEmpSpecialty, diasTrabalho: newEmpDiasTrabalho.join(','), avatar: `https://api.dicebear.com/7.x/notionists/svg?seed=${newEmpName}`
        });
        if (typeof success === 'string') { setErrorMessage(success); success = false; }
  ...
  const cancelEdit = () => {
    setEditingEmployeeId(null); setNewEmpName(''); setNewEmpEmail(''); setNewEmpSpecialty(''); setNewEmpRole('collaborator'); setNewEmpDiasTrabalho(['1','2','3','4','5','6']);
        ...
        <div className="fixed inset-0 bg-black/55 backdrop-blur-sm z-[70] p-4 flex items-center justify-center" onMouseDown={(event) => { if (event.target === event.currentTarget) setViewingEmployee(null); }}>
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-surface-container-low p-6 rounded-3xl border border-outline-variant/10 shadow-2xl w-full max-w-lg" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => { setNewEmpName(viewingEmployee.name); setNewEmpEmail(viewingEmployee.email); setNewEmpSpecialty(viewingEmployee.specialty); setNewEmpRole(viewingEmployee.role); setNewEmpDiasTrabalho((viewingEmployee.diasTrabalho || '1,2,3,4,5,6').split(',')); setEditingEmployeeId(viewingEmployee.id); setViewingEmployee(null); setShowEmployeeForm(true); }} className="px-4 py-2 rounded-xl bg-surface-container border border-outline-variant/20 text-xs font-bold uppercase tracking-widest text-on-surface hover:bg-surface-container-high transition-colors">Alterar</button>
