...
export function UpcomingAppointmentsModal({ open, appointments, employees, services, onClose }: UpcomingAppointmentsModalProps) {
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return appointments
      ...
      .filter((appointment) => {
        ...
        if (dateFilter === '7days' && (appointmentDate < sevenDaysAgo || appointmentDate > today)) return false;
        if (dateFilter === '30days' && (appointmentDate < thirtyDaysAgo || appointmentDate > today)) return false;
  ...
  const summary = useMemo(() => ({
    today: appointments.filter((appointment) => appointment.status === 'scheduled' && appointment.date === todayStr).length,
